"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.ViviendaService = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("typeorm");
const vivienda_entity_1 = require("./entities/vivienda.entity");
const typeorm_2 = require("@nestjs/typeorm");
let ViviendaService = class ViviendaService {
    constructor(viviendaRepository) {
        this.viviendaRepository = viviendaRepository;
    }
    async create(createViviendaDto) {
        const vivienda = this.viviendaRepository.create(createViviendaDto);
        await this.viviendaRepository.save(vivienda);
        return vivienda;
    }
    async findAll() {
        return this.viviendaRepository.find();
    }
    async findOne(id) {
        return this.viviendaRepository.findOneBy({ id });
    }
    async update(id, updateViviendaDto) {
        const updated = await this.viviendaRepository.update(id, updateViviendaDto);
        return updated.affected > 0 ? this.viviendaRepository.findOneBy({ id }) : null;
    }
    async remove(id) {
        const vivienda = await this.findOne(id);
        if (vivienda) {
            await this.viviendaRepository.remove(vivienda);
            return vivienda;
        }
        return null;
    }
};
exports.ViviendaService = ViviendaService;
exports.ViviendaService = ViviendaService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_2.InjectRepository)(vivienda_entity_1.Vivienda)),
    __metadata("design:paramtypes", [typeorm_1.Repository])
], ViviendaService);
//# sourceMappingURL=vivienda.service.js.map